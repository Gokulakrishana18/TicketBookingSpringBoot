...
public class ScreeningServiceImpl implements ScreeningService {
    ...
    @Override
    public List<Screening> getAuditoriumByTheaterId(Long theaterId) {
        return screeningRepository.findAll().stream().filter(screening ->
                screening.getAuditorium().getTheatre().getId()==theaterId)
                .collect(Collectors.toList());
    }

    @Override
    public Screening getScreeningById(Long screening_id) {
        return screeningRepository.findById(screening_id).
